...
const uint64_t TimeLimitListener::TIMER_TICKING_INTERVAL_US = 200 * 1000; // 200ms
const long TimeLimitListener::CLOCK_TICKS_PER_SECOND = withErrnoCheck("sysconf", sysconf, _SC_CLK_TCK);

TimeLimitListener::TimeLimitListener(
        uint64_t rTimelimitUs, uint64_t uTimelimitUs, uint64_t sTimelimitUs, uint64_t usTimelimitUs)
    : rTimelimitUs_(rTimelimitUs), uTimelimitUs_(uTimelimitUs), sTimelimitUs_(sTimelimitUs), usTimelimitUs_(usTimelimitUs), isTimerCreated_(false) {
...
void TimeLimitListener::onPostForkParent(pid_t childPid) {
    ...
    if (firstTimerTick != std::numeric_limits<uint64_t>::max()) {
        withErrnoCheck("timer_create", timer_create, CLOCK_MONOTONIC, nullptr, &timerId_);
        ...
        timerSpec.it_interval.tv_nsec = TIMER_TICKING_INTERVAL_US % 1000000 * 1000;
        withErrnoCheck("timer_settime", timer_settime, timerId_, 0, &timerSpec, nullptr);
    }
}
...
uint64_t TimeLimitListener::getRealTimeUsage() {
    ...
    uint64_t realTimeUsageUs = std::chrono::duration_cast<std::chrono::microseconds>(realTimeUsage).count();
