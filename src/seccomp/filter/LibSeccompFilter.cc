...
bool LibSeccompFilter::match(const tracer::TraceEvent& event, tracer::Tracee& tracee) const {
    return std::all_of(filterConditions_.begin(), filterConditions_.end(),
                       [&](auto& filter) { return filter(event, tracee); });
}

const std::vector<struct scmp_arg_cmp>& LibSeccompFilter::createLibSeccompFilter() const {
    return libSeccompFilterConditions_;
}

LibSeccompFilter LibSeccompFilter::operator&& (const LibSeccompFilter& filter) const {
    LibSeccompFilter newFilter;

    newFilter.filterConditions_ = filterConditions_;
    std::copy(filter.filterConditions_.begin(),
    ...
    std::copy(filter.libSeccompFilterConditions_.begin(),
...
SyscallArg::SyscallArg(uint8_t argumentIndex)
    : argumentIndex_(argumentIndex) {}

LibSeccompFilter SyscallArg::operator== (const uint64_t data) const {
    return LibSeccompFilter([index = this->argumentIndex_, data](const tracer::TraceEvent& event, tracer::Tracee& tracee) -> bool {
...
LibSeccompFilter MaskedSyscallArg::operator== (const uint64_t data) const {
    return LibSeccompFilter([index = this->argumentIndex_, mask = this->mask_, data](const tracer::TraceEvent& event,
                                                                                     tracer::Tracee& tracee) -> bool {
                                return (tracee.getSyscallArgument(index) & mask) == data;
                            },
                            SCMP_CMP(argumentIndex_, SCMP_CMP_MASKED_EQ, static_cast<uint64_t>(mask_), static_cast<uint64_t>(data)));
}
