...
public:
    ...
    SeccompRule(const std::string syscallName, SeccompAction&& action, SyscallFilter&& filter,
                typename std::enable_if<std::is_base_of<action::SeccompAction, SeccompAction>::value>::type* = nullptr,
                typename std::enable_if<std::is_base_of<filter::SyscallFilter, SyscallFilter>::value>::type* = nullptr)
        : syscall(resolveSyscallName(syscallName))
        , action(std::make_shared<SeccompAction>(std::forward<SeccompAction>(action)))
        , filter(std::make_shared<SyscallFilter>(std::forward<SyscallFilter>(filter))) {}

    template<typename SeccompAction>
    SeccompRule(const std::string syscallName, SeccompAction&& action,
                typename std::enable_if<std::is_base_of<action::SeccompAction, SeccompAction>::value>::type* = nullptr)
        : syscall(resolveSyscallName(syscallName))
        , action(std::make_shared<SeccompAction>(std::forward<SeccompAction>(action)))
